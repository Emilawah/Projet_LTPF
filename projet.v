...
(*
Apl(X,3) [1;2]
   -> evalA X [1;2] + evalA 3 [1.2]
   -> get 1 [1;2] + 3
   -> 2 + 3 -> 5
*)
Eval cbn in (get 1 [1;2]).
...
(*
                                         ...
                                         SN(i:=i-1)..    SN(x:=x+1)..
                                         ---------------------------- SN_Seq
      (refl)                                 SN corps [1;2] [0;3]                (refl)
   Cond=true                             ----------------------------          Cond=false
  -----------                                (s1=[0;3] calculé)              -------------- SN_While_False
  Cond [1;2]                                         |                       SN P1 [0;3]..
-------------------------------------------------------------------------------------------------- SN_While_True
                                     SN P1 [1; 2] [0; 3]
*)
...
Proof.
  ...
  - eapply SN_Seq.
    + apply SN_Assign.
    + eapply SN_Seq.
      apply SN_Assign. cbn. apply SN_Assign.
  - cbv. eapply SN_While_true.
    + cbn. reflexivity.
    + eapply SN_Seq. apply SN_Assign.
      eapply SN_Seq.
      * apply SN_Assign.
      * cbn. apply SN_Assign.
...
Fixpoint simpl_test_echange (i: winstr) : winstr :=
  ...
  | Seq i1 i2 => Seq (simpl_test_echange i1) (simpl_test_echange i2)
  | While b i1 => While b (simpl_test_echange i1)
  | If (Bnot b) i1 i2 => If b (simpl_test_echange i2) (simpl_test_echange i1)
  | If b i1 i2 => If b (simpl_test_echange i1) (simpl_test_echange i2)
  end.
...
Proof.
  intro b.
  induction b.
  - try( cbn[evalB]; destruct b; [ cbn | intro a | inversion a ]).
    cbn[evalB]. destruct b.
    + cbn. intro a. inversion a.
    + cbn. intro a. (* [....] *)
Admitted.
...
Proof.
  ...
  - destruct (f_SOS_1 i1 s).
    ...
    + apply SOS1_Seqf.
      apply IHi1.
  - destruct (evalB b s) eqn:Heq.
    + apply SOS1_If_true. apply Heq.
    + apply SOS1_If_false. apply Heq.
  - apply SOS1_While.
Qed.
