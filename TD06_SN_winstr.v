...
(** B1 :=  exp1 = 4 *)
Definition B1 := Beqnat E1 N4. (*x + 3 = 4*)

(** B2 := not ( bexp1 /\ (exp1 = 7) *)
Definition B2 := Bnot (Band B1 (Beqnat X N2)).

Compute(Beqnat E1 N4).
Compute(Beqnat X N2).
Compute(Band B1 (Beqnat X N2)). (* x+3 = 4 /\  x=2*   ==> 5=4 false*)
Compute(Bnot (Band B1 (Beqnat X N2))). (* B2 : not 5 = 4 ==> true*)
...
Proof.
  ...
  - cbn. Compute(evalB (Bnot (Beqnat (Ava 0) N0)) [0;y]).
    apply SN_While_false. cbn. reflexivity.
  - eapply SN_While_true.
    + cbn. reflexivity.
    + cbv[corps_boucle]. eapply SN_Seq.
      { apply SN_Assign. }
...
Proof.
  cbv. Compute(evalB(Bnot (Beqnat Ir N2)) [0;0;1]).
  eapply SN_While_true.
  - cbn. reflexivity.
  - eapply SN_Seq.
    + apply SN_Assign.
    + eapply SN_Seq. apply SN_Assign. apply SN_Assign.
  - cbv.  Compute(evalB(Bnot (Beqnat Ir N2)) [1;1;3]).
    eapply SN_While_true.
    + cbn. reflexivity.
    + eapply SN_Seq. apply SN_Assign. eapply SN_Seq.
         * cbn. apply SN_Assign.
         * cbn. apply SN_Assign.
    + cbn. Compute(evalB(Bnot (Beqnat Ir N2)) [2;4;5]).
      apply SN_While_false. cbn. reflexivity.
Qed.
...
Fixpoint simpl_test_Btrue_Bfalse (i: winstr) : winstr :=
  ...
  | If b i1 i2 => 
    If b (simpl_test_Btrue_Bfalse i1) 
...
Proof.
  ...
  *)
  ...
  - destruct cond; try (apply SN_If_false; [ apply e | apply hrec_sn ])
    + simpl in e. discriminate.
    + apply hrec_sn.
  - apply SN_While_false. apply H.
  - simpl in IHsn2. inversion IHsn2.
    + try (eapply SN_While_true; [ apply H | apply IHsn1 | apply IHsn2 ]).   
    + try (eapply SN_While_true; [ apply H | apply IHsn1 | apply IHsn2 ]).
Qed.
...
Fixpoint simpl_test_echange (i: winstr) : winstr :=
  ...
  | If Btrue i1 i2 => simpl_test_echange i1
  | If Bfalse i1 i2 => simpl_test_echange i2
  | If b i1 i2 => 
    If b (simpl_test_echange i1) 
      (simpl_test_echange i2)
  | While b i => While b (simpl_test_echange i)
  end.
...
Proof.
  ...
  cbn in p.
  destruct (evalB B s).
  - cbn in p. symmetry in p. rewrite p. cbn. reflexivity.
  - cbn in p. symmetry in p. rewrite p. cbn. reflexivity.
Qed.
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_While_true *)  cond i s s1 s' e sn1 hrec_sn1 sn2 hrec_sn2
                   ].
  - apply SN'_Skip.
  - apply SN'_Assign.
  - eapply SN'_Seq.
...
Definition corps_boucleR : rinstr := RSeq (RAssign Il (Amo Ir N1)) (RAssign Xl (Apl N1 Xr)).
...
Proof.
  cbv.
  (* on commence par false car au début i = 2*)
  ...
  (*cas ou i = 1 --> [1;6]*)    
  - cbn. reflexivity.
  (* On applique repeat a true car i = 0 , on sort de la boucle --> [0;6]*)
...
(*
                                                            ...
                                                            -------------- SNr_Assign   ------------- SNr_Assign
                                                                 i:=[1;6]                  x:=[0;6]
----------- SNr_Assign    -------------- SNr_Assign             --> [0;6]                 --> [0;7]                           
  i:=[2;5]                 x:=[1;5]                            ------------------------------------------- SNr_Seq            
 --> [1;5]                --> [1;6]                                    SNr corps_boucleR [1;6] [0;7]  
------------------------------------------ SNr_Seq            ---------------------------------------------- SNr_Repeat_true
     SNr corps_boucleR [2;5] [1;6]                                        SNr P2 [1;6] [0;7]
---------------------------------------------------------------------------------------------------------------  SNr_Repeat_false
                                         SNr P2 [2;5] [0;7]
...
Fixpoint repeat_while (i:rinstr) : winstr :=
   ...
   (*'Repeat i until b'peut être traduit  par : 'i; while (not b) do i*)
...
Proof.
  intros i s1 s2 sn.
  induction sn as  [(*Snr_Skip*) s
                  |(* SNr_Assign *) x e s
                  | (* SNr_Seq *) i1 i2 s s1 s2 sn1 hrec_sn1 sn2 hrec_sn2
                  | (* SNr_If_true *) b i1 i2 s s2 cond sn hrec_sn
                  | (* SNr_If_false *) b i1 i2 s s2 cond sn hrec_sn
                  | (* SNr_Repeat_true *) b i s s2 sn hrec_sn cond
                  | (* SNr_Repeat_false *) b i s s1 s2 sn1 hrec_sn1 cond sn2 hrec_sn2
                   ].
  - cbn. apply SN_Skip.
  - cbn. apply SN_Assign.  
  - cbn. eapply SN_Seq.
  ...
  - cbn. eapply SN_Seq.
    + apply hrec_sn.
    (*la condition est fause dans le while car :
       evalB b s2 = true
       et donc eval (Bnot b) s2 = false *)  
    + apply SN_While_false. cbn. rewrite cond. cbn. reflexivity.
  - cbn. eapply SN_Seq.
    + apply hrec_sn1.
    + cbn in hrec_sn2. apply inv_Seq in hrec_sn2. destruct hrec_sn2.
    (*la condition est vraie dans le while car :
       evalB b s1 = false
       et donc eval (Bnot b) s1 = true *) 
      eapply SN_While_true.
      * cbn. rewrite cond. cbn. reflexivity.
      * apply H.
      * apply H0.
Qed.
...
Fixpoint while_repeat (i:winstr) : rinstr :=
    ...
    (* Si b est faux on repeat tant que b est faux  , sinon on sort de repeat*)                    
